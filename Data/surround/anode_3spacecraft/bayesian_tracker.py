...
def savetrackedtypeiii(results, dir=f"./Data", date_str="date"):
    import pickle
    mkdirectory(dir+f"/{date_str}/Tracking/")

    with open(dir+f"/{date_str}/Tracking/"+f'/TRACKING_{date_str}_results_{N_STATIONS}stations.pkl', 'wb') as outp:
        pickle.dump(results, outp, pickle.HIGHEST_PROTOCOL)
...
def loadtypeiii(filenamef, spacecraft):
    ...
    if 'stereo' or 'stereoa' in spacecraft:
        stetypeiii = results['StereoTime']
        typeIII['stereo'] = stetypeiii
        loadedtxt = loadedtxt + ', stereo'
...
if __name__ == "__main__":
    ...
    for i_freq in list(typeIII_times):
        ...
        print(f"observed position: {xy}")
        results.append([float(i_freq), xy[0], xy[1]])
    ...
    #
    #
    #     results = Parallel(n_jobs=coresforloop, verbose=100)(delayed(parallel_tracker)
    #                     (i_freq,typeIII_times[i_freq],stations=stations)
    #                     for i_freq in list(typeIII_times))
