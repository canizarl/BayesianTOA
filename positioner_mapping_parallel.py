...
def plot_map_simple_withTracked(delta_obs, trackedtypeIII, xmapaxis, ymapaxis, stations,
                                vmin=0,vmax=30,v_sw=400,theta_sw=0, dtheta=0,
    ...
    im_track = ax.scatter(xy[:,1], xy[:,0],c = tracked_freqs, marker="s",edgecolors="w",cmap='plasma', s=250, label="TrackedBeam")
    i = 0
...
if __name__ == "__main__":
    ...
    if runserver == True:
        ...
        run_plotdata = False
        run_median_filter = False
        run_median_filter_TrackedRay = True
        run_datainterpolate = True
    ...
    if run_plotdata == True:
        ...
        plot_map_simple(delta_obs*2, xmapaxis, ymapaxis, stations_rsun,vmin=40,vmax=250, savefigure=True, date_str=date_str, filename=fname)
    """  ---------------------------------------------------------------------   """
    ...
    # from scipy.ndimage import median_filter as medfil
        ...
        plot_map_simple(median_filter_image, xmapaxis, ymapaxis, stations_rsun,vmin=40, vmax=250, savefigure=True, date_str=date_str,filename=fname)
    """  ---------------------------------------------------------------------   """
    ...
    if run_median_filter_TrackedRay == True:
        ...
        plot_map_simple_withTracked(median_filter_image,trackedtypeIII, xmapaxis, ymapaxis, stations_rsun,vmin=np.min(median_filter_image), vmax=np.max(median_filter_image),v_sw=300,
    ...
    if run_datainterpolate == True:
        ...
        tracked = loadtrackedtypeiii(trackedfile)
        plot_map_simple_withTracked(znew,trackedtypeIII, xnew, ynew, stations_rsun,vmin=np.min(median_filter_image), vmax=250,v_sw=300,
                                    theta_sw=160, dtheta=20, savefigure=True, date_str=date_str, filename=fname)
